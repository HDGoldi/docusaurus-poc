...
* Device metadata can be injected in the Webhook's URL and headers. See more details [here](doc:cloud-integrator-webhook-configuration#metadata-injection-in-webhook-definitions).
* Integrations can be tested by sending TEST_MESSAGE [event type](doc:cloud-integrator-output-format). This can be done by using [Test AWS Integration](ref:post_v1-integrate-clouds-aws-integrationid-test) or [Test Webhook Integration](ref:post_v1-integrate-clouds-webhooks-integrationid-test) endpoints.
...
* In case of Cloud Integration Failure, special Error Admin Log entry will be created, which can be used for monitoring [Cloud Integration failure events](doc:cloud-integrator-failure-event).
