...
In this example, we declare a data conversion used with a home alarm system device. When the device senses motion it sends one byte to [UDP endpoint](/docs/1nce-os/1nce-os-device-integrator/device-integrator-udp/). This simple template converts that one byte sent by the device into a JSON object with a boolean field called `motion`. 
