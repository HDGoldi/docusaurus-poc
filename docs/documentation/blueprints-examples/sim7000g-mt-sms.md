...
After connecting to the network, wait until the issued MT-SMS is received by the device. By default this is indicated by '+CMTI: "SM",`<number_unread>`' returned from the SIM7000G.
...
MT-SMS messages issued through the API of 1NCE portal, received by the SIM7000G can be read using a few simple AT Commands. Setting up the APN is not required for using SMS.
