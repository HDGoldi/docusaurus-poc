...
* Public IP addresses (`https://"server-ip":"port"/"endpoint"/`) are not supported.
* Custom ports for the endpoint can be configured via the URL (`https://"server-domain":"port"/"endpoint"/`). 
