...
In the table view below the sent SMS form, an overview of the Mobile Originated (MO) and Mobile Terminated (MT) SMS messages of the last seven days can be seen. To properly receive and process MT messages, review the [SMS Forwarder Service](/platform-services/platform-services-sms-forwarder/index) guide. Besides the messages itself, the current status of the MO-/MT-SMS, the payload and type is listed.  
